...
if not st.session_state.logged_in:
    ...
    if tab == "Disease Predictions":
        ...
        #Pnuemonia Prediction
        if selected == 'Pnuemonia Prediction':
            # page title
            ...
            if uploaded_file is not None:
               ...
               img_array = tf.keras.preprocessing.image.img_to_array(img)  # Fixed: using 'img' instead of 'image'
               ...
               if prediction > 0.5:
                   diagnosis = "Pnuemona detected 😷"
                   st.error("Prediction: Pneumonia Detected 😷")
               else:
                   diagnosis = "Normal Chest X-ray ✅"
                   st.success("Prediction: Normal Chest X-ray ✅")
               # ✅ Console log for debugging
               print(f"[DEBUG] Saving result: {user[1]} - Pnuemonia - {diagnosis}")
               save_result(username=user[1], disease="Pnuemonia", result=diagnosis)    
